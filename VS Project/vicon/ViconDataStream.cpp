...
int readVicon(double** quaternion, int nImu, bool * g_b_viconLoop)
{
	int argc_ = 0;
	char** argv_ = NULL;
  // Program options
...
//  std::string HostName = "localhost:801";
  ...
  for(int a=2; a < argc_; ++a)
  {
    ...
    else if (arg=="--enable_haptic_test")
    {
      ...
      {    
        ...
        while( a < argc_ && strncmp( argv_[a], "--", 2 ) !=0  )
        {
          HapticOnList.push_back( argv_[a] );
          ++a;
        }
      }
    }
    ...
    {
      std::cout << "Failed to understand argument <" << argv_[a] << ">...exiting" << std::endl;
      return 1;
    }
  }
  ...
  for(int i=0; i != 3; ++i) // repeat to check disconnecting doesn't wreck next connect
  {
	...
	int test = 0;
    while( !MyClient.IsConnected().Connected && test<5)
    {
		test++;
      // Direct connection
	...
	std::cout << "START VICON LOOP! ";

  #ifdef WIN32
    while( (*g_b_viconLoop) && !Hit() )
	  ...
	  for( unsigned int SubjectIndex = 0 ; SubjectIndex < SubjectCount ; ++SubjectIndex )
      {
		...
		*/
		  ...
		  for (int i = 4; i < 7; i++)
		  {
			  ...
			  quaternion[SubjectIndex][i] = _Output_GetSegmentGlobalTranslation.Translation[i-4];
			//  std::cout << "SegmentIndex"<< SegmentIndex<< " "<< quaternion[SegmentIndex][i]<<std::endl;
		  ...
		  for (int i = 0; i < 4; i++)
		  {
			  ...
			  quaternion[SubjectIndex][i] = _Output_GetSegmentGlobalRotationQuaternion.Rotation[i];
			 // std::cout << quaternion[SegmentIndex][i];
